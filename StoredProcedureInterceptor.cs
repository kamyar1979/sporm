			...
			try
			{
				...
				foreach (var item in invocation.Method.GetParameters())
				{
                    ...
                    else
                    {
						...
						param.ParameterName = Attribute.IsDefined(item, typeof(DbNameAttribute)) ?
							(Attribute.GetCustomAttribute(item, typeof(DbNameAttribute)) as DbNameAttribute)?.Name
							: item.Name;
						...
						{
							param.Size = (Attribute.GetCustomAttribute(item, typeof(SizeAttribute)) as SizeAttribute)!.Size;
						}
				...
				if (invocation.Method.ReturnType != typeof(void))
				{
					if (!returnAsResult)
					{
						if (invocation.Method.ReturnType == typeof(Dictionary<string, object>))
						{
							...
							if (_reader.Read())
							{
								...
								{
									var instance = fields.ToDictionary(name => name, name => _reader[name] is DBNull ? null : _reader[name]);
						...
						else if (invocation.Method.ReturnType.GetInterface("IEnumerable") != null)
						{
							...
							if (invocation.Method.ReturnType.IsGenericType && invocation.Method.ReturnType.GetGenericArguments()[0] != typeof(object))
							{
								...
								else
								{
									...
									invocation.ReturnValue = typeof(Utils).GetMethod("GetIterator", BindingFlags.Static | BindingFlags.NonPublic)!.MakeGenericMethod(type).Invoke(null, [_reader]);
								}
							}
							else
							{
							...
							using (_reader = command.ExecuteReader())
							{
								if (_reader.Read())
								{
									...
									if (invocation.Method.ReturnType == typeof(object))
									{
										...
										foreach (var name in fields)
										{
											type.GetProperty(name)!.SetValue(instance, _reader[name] is DBNull ? null : _reader[name], null);
										}
										invocation.ReturnValue = instance;
									}
									else
									{
										...
										foreach (var prop in invocation.Method.ReturnType.GetProperties())
										{
											...
											{
												prop.SetValue(instance, _reader[prop.Name] is DBNull ? null : _reader[prop.Name], null);
											}
										}
										invocation.ReturnValue = instance;
									}
					...
					else
					{
						...
						returnValue.DbType = (DbType)Enum.Parse(typeof(DbType), invocation.Method.ReturnType.Name.Replace("&", ""));
